...
namespace DepotDownloader
{
    public class ContentDownloader2
    {
        ...
        private readonly string STAGING_DIR = Path.Combine(CONFIG_DIR, "staging");
        ...
        public async Task DownloadAppAsync(uint appId, List<(uint depotId, ulong manifestId)> depotManifestIds, string branch, string os, string arch, string language, bool lv, bool isUgc)
        {
        ...
        DepotDownloadInfo GetDepotInfo(uint depotId, uint appId, ulong manifestId, string branch)
        {
        ...
        private async Task DownloadSteam3AsyncDepotFiles(CancellationTokenSource cts, uint appId,
            GlobalDownloadCounter downloadCounter, DepotFilesData depotFilesData, HashSet<String> allFileNamesAllDepots)
        {
            ...
            WriteLog($"Downloading depot {depot.id}", ContentDownloader2LogType.TaskMessage);
            ...
            else
            {
                ...
                if (neededChunks.Count() == 0)
                {
                    lock (depotDownloadCounter)
                    {
                        ...
                        DownloadProgress((depotDownloadCounter.SizeDownloaded / (float)depotDownloadCounter.CompleteDownloadSize) * 100.0f, fileFinalPath);
                    }
        ...
        void DumpManifestToTextFile(DepotDownloadInfo depot, ProtoManifest manifest)
        {
        ...
        private void WriteLog(string logMessage, ContentDownloader2LogType logType, params object?[] args)
        {
            LogEntryEvent?.Invoke(this, new ContentDownloader2LogEntryArgs() { LogMessage = string.Format(logMessage, args), LogType = logType });
        }
        private void DownloadProgress(float progress, string file)
        {
            DownloadProgressEvent?.Invoke(this, new ContentDownloader2DownloadProgressArgs() { Progress = progress, File = file });
        }
        public delegate void LogEntryEventHandler(object sender, ContentDownloader2LogEntryArgs e);
        public delegate void DownloadProgressEventHandler(object sender, ContentDownloader2DownloadProgressArgs e);
        public event LogEntryEventHandler LogEntryEvent;
        public event DownloadProgressEventHandler DownloadProgressEvent;
    }

    public enum ContentDownloader2LogType
    {
        TaskMessage,
        Information,
        Warning,
    ...
    public class ContentDownloader2LogEntryArgs : EventArgs
    {
        public string LogMessage { get; set; }
        public ContentDownloader2LogType LogType { get; set; }
    }
    public class ContentDownloader2DownloadProgressArgs : EventArgs
    {
